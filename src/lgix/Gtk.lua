...
Gtk.Container._accessor = { children = {}, child = {} }
-- Reading yields the table of all children.
Gtk.Container._accessor.children.get = Gtk.Container.get_children

-- Writing adds new children, optionally with specified child
-- properties.
function Gtk.Container._accessor.children.set(container, children)
...
-- on specified child.
function Gtk.Container._accessor.child.get(container)
   return setmetatable({ _container = container }, container_prop_children_mt)
end

-- Writing adds new child.
Gtk.Container._accessor.child.set = container_add_child
...
Gtk.TreeModel._accessor = { values = {} }
function Gtk.TreeModel._accessor.values.get(model)
...
Gtk.ListStore._accessor = { values = {} }
function Gtk.ListStore._accessor.values.get(model)
...
Gtk.TreeViewColumn._accessor = { cell = {} }
function Gtk.TreeViewColumn._accessor.cell.set(column, cell)
...
Gtk.TreeView._accessor = { columns = {} }
function Gtk.TreeView._accessor.columns.set(treeview, columns)
   for i = 1, #columns do Gtk.TreeView.append_column(treeview, columns[i]) end
end

-------------------- Gtk.Dialog

Gtk.Dialog._accessor = { buttons = {}, flags = {} }
function Gtk.Dialog._accessor.buttons.set(dialog, buttons)
...
Gtk.FileChooser._accessor = { file = {}, filename = {} }
function Gtk.FileChooser._accessor.file.get(filechooser)
   return Gtk.FileChooser.get_file(filechooser)
end

function Gtk.FileChooser._accessor.file.set(filechooser, file)
   return Gtk.FileChooser.set_file(filechooser, file)
end

function Gtk.FileChooser._accessor.filename.get(filechooser)
   return Gtk.FileChooser.get_filename(filechooser)
end

function Gtk.FileChooser._accessor.filename.set(filechooser, filename)
   return Gtk.FileChooser.set_filename(filechooser, filename)
end

-------------------- Gtk.FileFilter

Gtk.FileFilter._accessor = { name = {} , pattern = {}, mime_type = {} }
function Gtk.FileFilter._accessor.name.get(filefilter)
   return Gtk.FileFilter.get_name(filefilter)
end

function Gtk.FileFilter._accessor.name.set(filefilter, name)
...
function Gtk.FileFilter._accessor.pattern.set(filefilter, pattern)
   filefilter_add(filefilter, pattern, Gtk.FileFilter.add_pattern)
end

function Gtk.FileFilter._accessor.mime_type.set(filefilter, mime_type)
...
Gtk.Builder._accessor = { objects = {}, file = {}, string = {}, connect = {} }
function Gtk.Builder._accessor.objects.get(builder)
...
function Gtk.Builder._accessor.file.set(builder, vals)
   builder_add(builder, vals, Gtk.Builder.add_from_file)
end

function Gtk.Builder._accessor.string.set(builder, vals)
   builder_add(builder, vals, Gtk.Builder.add_from_string)
end

function Gtk.Builder._accessor.connect.set(builder, target)
